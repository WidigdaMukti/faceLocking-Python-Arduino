...
while q<=2:
    data_path = 'D:/program/python/FACELOCKING-ARDUINO+PYTHON/images/'
...
while True:
    ...
    try:
        ...
        if result[1]<500:
            ...
            cv2.putText(image, display_string,(100,120),cv2.QT_FONT_NORMAL,0.5,(255,255,255))

        if confidence>=83:
            cv2.putText(image,"unlocked",(250,450),cv2.QT_FONT_NORMAL,1,(255,255,255))
            cv2.imshow('face recognize',image)
            x+=1
        else:
            cv2.putText(image,"locked",(250,450),cv2.QT_FONT_NORMAL,1,(255,255,255))
            cv2.imshow('face recognize',image)
            c+=1
    except:
        cv2.putText(image,"Face not found",(250,450),cv2.QT_FONT_NORMAL,1,(255,255,255))
        cv2.imshow('face recognize',image)
        d+=1
        pass
